...
export async function generateMetadata({ params }: { params: Promise<{ name: string }> }): Promise<Metadata> {
  ...
  const ogImageUrl = `https://clawharbor.work/api/og?name=${encodeURIComponent(name)}`;
  
  return {
    title: `${displayName}'s Trading Card — clawharbor`,
    ...
    openGraph: {
      ...
      url: `https://clawharbor.work/card/${name}`,
      siteName: 'clawharbor',
      type: 'website',
      images: [{
        url: ogImageUrl,
        width: 1200,
        height: 630,
        alt: `${displayName}'s clawharbor Trading Card`,
      }],
    },
    twitter: {
