...
export default function HomePage() {
  ...
  useEffect(() => {
    const seen = sessionStorage.getItem('clawharbor-boot-seen');
    if (!seen) {
      sessionStorage.setItem('clawharbor-boot-seen', 'true');
      setShowBoot(true);
    }
    if (localStorage.getItem('clawharbor-sfx') === 'on') setSfxEnabled(true);
  ...
  useEffect(() => {
    ...
    window.addEventListener('clawharbor-agent-card', handleAgentCard);
    return () => window.removeEventListener('clawharbor-agent-card', handleAgentCard);
  }, [agents]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const starsPromise = (() => {
        try {
          const cached = localStorage.getItem('clawharbor-stars-cache');
          if (cached) {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{
            ...
            {isMobile ? 'clawharbor' : 'clawharbor'}
          </h1>
          <span style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              ...
              const tweetUrl = encodeURIComponent('https://clawharbor.work/?demo=true');
