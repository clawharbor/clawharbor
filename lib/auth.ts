...
const TOKEN_FILE = path.join(os.homedir(), '.openclaw', '.clawharbor-token');
 ...
 * Simple file-based storage in ~/.openclaw/.clawharbor-token
 */
...
export function verifyToken(request: Request): boolean {
  ...
  const providedToken = request.headers.get('X-clawharbor-Token');
