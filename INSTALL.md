...
**Get clawharbor running in under 5 minutes.**

---

## Prerequisites

You need [OpenClaw](https://openclaw.ai) installed first. clawharbor is a skill/UI for OpenClaw — it won't work without it.
...
clawharbor auto-detects agents from OpenClaw config. To customize names, colors, roles:
...
Restart clawharbor (`Ctrl+C`, then `npm run dev`) to see changes.
...
clawharbor reads session files to detect activity. If agents are working in OpenClaw but showing idle here:
...
2. Join [Discord](https://discord.gg/clawd) — #clawharbor channel
3. Open an issue: https://github.com/clawharbor/clawharbor/issues
...
Want to run clawharbor 24/7 on a server?

### Option 1: PM2 (Recommended)
```bash
npm install -g pm2
cd ~/clawharbor
pm2 start npm --name "clawharbor" -- run dev
pm2 save
pm2 startup  # Follow instructions to enable on boot
```

### Option 2: systemd (Linux)
Create `/etc/systemd/system/clawharbor.service`:
```ini
[Unit]
Description=clawharbor
After=network.target

[Service]
Type=simple
User=youruser
WorkingDirectory=/home/youruser/clawharbor
...
Enable and start:
```bash
sudo systemctl enable clawharbor
sudo systemctl start clawharbor
```

### Option 3: Docker (Coming Soon)
```bash
# Not yet available — v0.2 will ship with Dockerfile
docker run -p 3333:3333 clawharbor/clawharbor:latest
```
...
**Questions?** [Discord](https://discord.gg/clawd) | [Docs](https://docs.openclaw.ai) | [Issues](https://github.com/clawharbor/clawharbor/issues)
