...
/**
 * Send a message to an agent via the openclaw gateway and get their response.
 * Returns null if the binary is unavailable or the call fails.
 */
...
/**
 * Fallback: generate a water cooler reply via Surplus Intelligence API.
 * Used when the openclaw binary is unavailable or returns nothing.
 */
async function getSurplusReply(
  speaker: AgentInfo,
  allAgents: AgentInfo[],
  recentChat: any[],
  mission: any,
  ...
  if (!isSurplusConfigured()) return null;

  try {
    const prompt = buildPrompt(speaker, allAgents, recentChat, mission, accomplishments);
    const raw = await surplusChat(
      [{ role: 'user', content: prompt }],
      { max_tokens: 300 },
    );
    return raw.trim() || null;
  } catch (err) {
    console.warn('[chat] Surplus fallback failed:', (err as Error).message);
    return null;
  }
}

/**
 * Clean up an LLM reply to be suitable for the water cooler.
 * Strips markdown, name prefixes, surrounding quotes, and truncates.
 */
function cleanReply(raw: string, speakerName: string): string {
  let text = raw;
  ...
  // Take only the first meaningful paragraph
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Allow adding a user message to the chat (accept from/text or agent/message, validate non-empty)
    ...
    // Primary path: openclaw binary
    markWatercoolerActive(speaker.id);
    const openclawReply = getAgentReply(speaker.id, buildPrompt(speaker, allAgents, chat, config.mission, accomplishments));
    markWatercoolerActive(speaker.id);

    // Fallback path: Surplus Intelligence API when openclaw returns nothing
    let rawReply: string | null = openclawReply;
    if (!rawReply) {
      rawReply = await getSurplusReply(speaker, allAgents, chat, config.mission, accomplishments);
    }

    if (rawReply) {
      const text = cleanReply(rawReply, speaker.name);
