...
/**
 * Battle API — Generate agent debate arguments
 *
 ...
 *   2. Surplus Intelligence  (requires SURPLUS_API_KEY env var, fallback)
 *
 * If both providers fail or neither key is configured the route returns a
 * structured error so the UI can show its built-in fallback arguments.
 */
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.warn('[battle] Bankr failed, trying Surplus:', (bankrErr as Error).message);
      ...
      if (isSurplusConfigured()) {
        try {
          ...
          console.error('[battle] Surplus also failed:', (surplusErr as Error).message);
        }
      }
    }
