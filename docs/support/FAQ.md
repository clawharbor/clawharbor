...
Common questions about installing, using, and customizing clawharbor.

---

## Installation & Setup

### Q: I installed clawharbor but my agents aren't showing up. What's wrong?

**A:** clawharbor auto-discovers agents from `~/.openclaw/openclaw.json`. Check:
...
3. **Are your agents running?**
   Agents appear in clawharbor once they've had at least one session. Send a message to wake them up:
   ```bash
...
The new agent will appear in clawharbor automatically!
...
Or use `clawharbor.config.json` for more options:
```json
{
  "agents": {
    "my-agent": {
...
**To set up cooldowns:**
1. Create `clawharbor.config.json` with cooldown settings:
   ```json
   {
     "cooldown": {
       "default": "10m",
...
**A:** Change the port:
```bash
PORT=4000 clawharbor
# or
clawharbor --port=4000
```
...
**A:** Two locations:
1. **Project-specific:** `~/clawharbor/clawharbor.config.json` (recommended for development)
2. **Global:** `~/.openclaw/clawharbor.config.json` (recommended for production)

clawharbor checks both and uses the first one it finds.
...
**A:** Yes! In `clawharbor.config.json`:
```json
{
  "waterCooler": {
    "enabled": false
...
**A:** Demo mode lets you try clawharbor with fake data before installing it. Visit:
```
http://localhost:3333?demo=true
```

You'll see 5 sample agents working, chatting, and completing quests. Great for showing others what clawharbor does!
...
### Q: How do I contribute to clawharbor?
...
### Q: How do I run clawharbor locally for development?
...
### Q: How does clawharbor detect when agents are working?
...
### Q: Can I integrate clawharbor with other tools?
...
- **GitHub Issues:** [Report bugs or request features](https://github.com/clawharbor/clawharbor/issues)
- **GitHub Discussions:** [Ask questions or share ideas](https://github.com/clawharbor/clawharbor/discussions)
