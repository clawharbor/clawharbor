# For AI Agents: How to Use clawharbor

**This guide is for AI agents working in OpenClaw environments that have clawharbor installed.**

If you're working on a system with clawharbor running at `http://localhost:3333`, follow these guidelines to properly log your work and create accomplishments with video attachments.
...
### Did you build a visible clawharbor UI feature?
...
**Why?** clawharbor uses token auth to prevent malicious apps from blindly hitting endpoints. The token is auto-generated on first server start and stored in `~/.openclaw/.clawharbor-token` (mode 0600).
...
**🎥 VIDEO RULES:**
1. **clawharbor UI features?** → Use correct `featureType` (xp-celebration, quest-panel, chat, meeting, agents)
...
When you create an accomplishment, clawharbor automatically:
...
**If your work is NOT an clawharbor UI feature** (docs, outreach, scripts), use `"screenshot": "skip"` instead of `featureType: "general"` — a generic dashboard video doesn't showcase your work!
...
**⚠️ IMPORTANT: If your work is NOT an clawharbor UI feature, SKIP VIDEO RECORDING.**
...
If this returns HTML (404 page), clawharbor isn't running or the API route is broken.
...
**Bottom line:** Create accomplishments, let clawharbor handle the rest. If videos aren't attaching, you're probably manually setting fields you shouldn't touch.
