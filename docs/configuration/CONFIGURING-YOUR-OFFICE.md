...
Create `clawharbor.config.json` in your clawharbor directory (or `~/.openclaw/clawharbor.config.json` for global config):
...
### Default Behavior
- clawharbor reads cooldown timers from your OpenClaw cron jobs (`~/.openclaw/cron/jobs.json`)
...
When you set cooldowns in the config, clawharbor creates OpenClaw cron jobs for each agent automatically:
