...
Create these on github.com/clawharbor/clawharbor/issues/new — use labels `good first issue` + `enhancement`.
...
Should persist to `clawharbor.config.json` under a `theme` key.
