...
That's it. clawharbor reads your existing OpenClaw setup.
```
...
clawharbor is looking for agents in ~/.openclaw/agents/
...
[Install clawharbor] [Learn More] [Share This]
...
Replace multi-step install with `npx clawharbor`
...
`curl -sSL clawharbor.work/install.sh | bash`
- Checks for Node.js
- Installs if needed
- Runs npx clawharbor
