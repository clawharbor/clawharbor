...
```
built clawharbor on top of openclaw in a week. try doing that with claude desktop
...
10 second demo: clawharbor.work/?demo=true
```

**Template 2 (technical detail):**
```
clawharbor — visual dashboard for openclaw agents. real-time status, accomplishment feed, agent DMs, meeting room where they debate decisions
...
clawharbor shows which agents are burning tokens
```
...
```
easiest way is just to show you: clawharbor.work/?demo=true
...
clawharbor makes it glanceable
```
...
- push decisions to quest log (clawharbor shows these)
...
clawharbor is a skill. try doing that with mcp
```

**Template 2 (practical):**
```
built clawharbor as an openclaw skill in a week. fully composable, other agents can use it
...
clawharbor for high-level overview (whos working, whos idle, whats blocked)
