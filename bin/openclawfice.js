...
if (!fs.existsSync(openclawConfig)) {
  console.error('\n❌ OpenClaw not found!\n');
  console.error('clawharbor requires OpenClaw to be installed and configured.');
...
const firstRunMarker = join(statusDir, '.clawharbor-init');
...
if (!fs.existsSync(firstRunMarker)) {
  ...
  if (!fs.existsSync(join(statusDir, 'chat.json'))) {
    ...
    const chat = [
      { from: firstAgent, text: '🏢 Office is open! Welcome to clawharbor.', ts: now - 300000 },
    ...
    const accomplishments = [
      {
        ...
        icon: '🏢',
        title: 'Office opened!',
        detail: 'clawharbor installed and running. Your agents are ready to work.',
        who: firstAgent,
        timestamp: now,
      }
    ];
  ...
  const tokenFile = join(openclawDir, '.clawharbor-token');
  if (!fs.existsSync(tokenFile)) {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        execSync(`security add-generic-password -s "clawharbor" -a "api-token" -w "${token}"`, { stdio: 'ignore' });
  ...
  // Hint about config
  console.log('\n  💡 Tip: Create clawharbor.config.json to customize NPC colors.');
  console.log('     See clawharbor.config.example.json for reference.\n');
...
const versionFile = join(statusDir, '.clawharbor-version');
...
if (currentVersion !== lastVersion) {
  console.log(`🔄 clawharbor updated (${lastVersion} → ${currentVersion}), running migrations...\n`);
...
if (command === 'doctor' || command === '--doctor') {
  ...
  // 4. Auth token check
  const tokenPath = join(homeDir, '.openclaw', '.clawharbor-token');
  ...
  req.on('error', (err) => {
    if (err.code === 'ECONNREFUSED') {
      ...
      warnings.push('Start the server: clawharbor');
    } else if (err.code === 'EADDRINUSE') {
      console.log(`❌ Port ${checkPort} is in use by another process`);
      issues.push(`Kill the process or use: clawharbor --port=<other-port>`);
    } else {
...
if (command === 'status' || command === '--status') {
  ...
  console.log('\n🏢 clawharbor — Status Report');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  
  // Get auth token
  let token = null;
  const tokenFile = join(homeDir, '.openclaw', '.clawharbor-token');
  if (fs.existsSync(tokenFile)) {
  ...
  const headers = token ? { 'X-clawharbor-Token': token } : {};
  ...
  http.get(url, { headers }, (res) => {
    ...
    console.log(`  clawharbor${pArg ? ' ' + pArg : ''}`);
    console.log('');
    process.exit(1);
  });
...
if (command === 'uninstall' || command === '--uninstall') {
  ...
  rl.question('⚠️  This will remove clawharbor completely. Continue? (y/N) ', (answer) => {
    ...
    const { execSync } = require('child_process');
    const installDir = join(homeDir, 'clawharbor');
    const launcherPath = join(homeDir, '.local', 'bin', 'clawharbor');
    
    try {
      // Remove installation directory
      if (fs.existsSync(installDir)) {
        console.log('  🗑️  Removing ~/clawharbor...');
        fs.rmSync(installDir, { recursive: true, force: true });
      }
      
      // Remove launcher script
      if (fs.existsSync(launcherPath)) {
        console.log('  🗑️  Removing ~/.local/bin/clawharbor...');
      ...
      console.log('clawharbor has been uninstalled.\n');
      console.log('To reinstall: curl -fsSL https://clawharbor.work/install.sh | bash\n');
...
if (command === 'help' || command === '--help' || command === '-h') {
  console.log(`
🏢 clawharbor — Virtual Office Dashboard for OpenClaw

Usage:
  clawharbor [command] [options]
...
Commands:
  ...
  uninstall         Remove clawharbor completely (-100 XP)
  help              Show this help
...
Examples:
  clawharbor                    # Start server on port 3333
  clawharbor status             # Check office health (RPG-style)
  clawharbor doctor             # Diagnose common issues
  clawharbor --port=8080        # Start server on port 8080
  clawharbor update             # Upgrade to latest version
  clawharbor uninstall          # Remove clawharbor
  clawharbor sync-cooldowns     # Sync cooldown timers

Visit: https://docs.openclaw.ai/clawharbor
  `);
  process.exit(0);
}
...
console.log('🏢 Starting clawharbor...\n');
