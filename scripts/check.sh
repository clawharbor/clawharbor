#!/bin/bash
# clawharbor Health Check — unified verification script
...
BASE_PROD="https://clawharbor.work"

echo ""
echo -e "${BOLD}🏥 clawharbor Health Check${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [ "$MODE" = "local" ] || [ "$MODE" = "all" ]; then
  ...
  if [ -f "public/clawharbor-demo.gif" ]; then
    size=$(du -k "public/clawharbor-demo.gif" | cut -f1)
    ok "Demo GIF: ${size}KB"
  else
...
if [ "$MODE" = "prod" ] || [ "$MODE" = "all" ]; then
  echo ""
  echo -e "${BOLD}🌐 Production (clawharbor.work)${NC}"
