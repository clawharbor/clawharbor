#!/bin/bash
# Smart feature recording - focuses on clawharbor and captures just that window
...
# Function to get clawharbor window ID
get_window_id() {
...
# Try to find clawharbor window
WINDOW_ID=$(get_window_id)

if [ -n "$WINDOW_ID" ] && [ "$WINDOW_ID" != "" ]; then
  # Record just the clawharbor window (not entire screen)
  ...
  echo "WARNING: Could not find clawharbor window, capturing full screen" >&2
  # Fallback: capture entire screen
  /usr/sbin/screencapture -V "$DURATION" -x "$VIDEO_FILE" 2>/dev/null
fi
