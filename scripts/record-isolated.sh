#!/bin/bash
# Record clawharbor in an isolated browser window (Puppeteer/Playwright)
...
(async () => {
  ...
  try {
    ...
    const tempFramesDir = `/tmp/clawharbor-frames-${outputName}`;
...
if ! node -e "require('puppeteer')" 2>/dev/null; then
  ...
  echo "Or: cd ~/clawd-clawharbor/clawharbor && npm install puppeteer" >&2
  exit 1
fi
