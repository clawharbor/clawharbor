#!/usr/bin/env bash
# Get clawharbor auth token for API calls
#
# Usage:
...
if [ -f ~/.openclaw/.clawharbor-token ]; then
...
echo "Error: Could not get clawharbor token" >&2
echo "Make sure clawharbor is running: npm run dev" >&2
exit 1
